"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const chai_1 = require("chai");
const axios_1 = __importDefault(require("axios"));
const mocha_1 = require("mocha");
const api = axios_1.default.create({
    baseURL: 'http://localhost:3001/api', // Remplacez par l'URL de votre serveur Express.js
});
let token;
let user_id;
let product_id;
let order_id;
let review_id;
describe('/ping', () => {
    it('should return pong', () => __awaiter(void 0, void 0, void 0, function* () {
        (0, chai_1.expect)('pong').equal('pong');
    }));
});
describe('Create User Route', () => {
    it('Devrait créer un nouvel utilisateur avec les informations fournies', () => __awaiter(void 0, void 0, void 0, function* () {
        const userPayload = {
            username: 'admin',
            password: '0000',
            phone: '0664674495',
            email: 'mathisbrouard@gmail.com',
            roleId: 1,
            address: {
                street: 'Rue ane',
                zipcode: 61000,
                country: 'France',
                number: 2,
                city: 'Alencon',
            },
        };
        const response = yield api.post('/user', userPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        (0, chai_1.expect)(response.data.username).to.equal(userPayload.username);
        user_id = response.data.id;
    }));
});
describe('Login Route', () => {
    it('Devrait retourner un token JWT valide', () => __awaiter(void 0, void 0, void 0, function* () {
        const loginPayload = {
            email: 'mathisbrouard@gmail.com',
            password: '0000',
        };
        const response = yield api.post('/login', loginPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('token');
        token = response.data.token;
    }));
});
describe('Put User Route', () => {
    (0, mocha_1.beforeEach)('Assurer la connexion et la création d\'utilisateur', () => __awaiter(void 0, void 0, void 0, function* () {
        const loginPayload = {
            email: 'mathisbrouard@gmail.com',
            password: '0000',
        };
        const loginResponse = yield api.post('/login', loginPayload);
        token = loginResponse.data.token;
    }));
    it('Devrait modifier un utilisateur', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const putPayload = {
            username: "adminModified",
        };
        const response = yield api.put(`/user/update`, putPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('username');
        (0, chai_1.expect)(response.data.username).to.equal(putPayload.username);
    }));
});
describe('Create product Route', () => {
    it('Devrait créer un nouveau produit avec les informations fournies', () => __awaiter(void 0, void 0, void 0, function* () {
        const productPayload = {
            name: "Un vélo tout simple",
            brand: "Une marque toute simple",
            weight: 14000,
            description: "Une description toute simple pour un vélo tout simple",
            price: 499,
            quantity: 1000,
            category: 1,
            image: "data:image/png;base64,"
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.post('/product', productPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        product_id = response.data.id;
    }));
});
describe('Update Product Route', () => {
    it('Devrait mettre à jour un produit', () => __awaiter(void 0, void 0, void 0, function* () {
        const productPayload = {
            name: "Un vélo tout simple2",
            brand: "Une marque toute simple2",
            weight: 140001,
            description: "Une description toute simple pour un vélo tout simple2",
            price: 501,
            quantity: 1001,
            category: 1
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.put(`/products/${product_id}`, productPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        product_id = response.data.id;
    }));
});
describe('Get Product filtered Route', () => {
    it('Devrait récupérer un tableau de produit selon un filtre', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.get(`/products?name=Un%20vélo%20tout%20simple2&category=1&min_price=500`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.be.an('array');
        (0, chai_1.expect)(response.data).to.have.lengthOf.to.greaterThanOrEqual(1);
    }));
});
describe('Create order Route', () => {
    it('Devrait créer une commande de produit', () => __awaiter(void 0, void 0, void 0, function* () {
        const orderPayload = {
            orders: [{
                    productId: product_id,
                    quantity: 1
                }]
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.post('/order', orderPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data[0]).to.have.property('id');
        order_id = response.data[0].id;
    }));
});
describe('Increase order count Route', () => {
    it('Devrait modifier une commande de produit', () => __awaiter(void 0, void 0, void 0, function* () {
        const orderPayload = {
            id: order_id,
            increase: true
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.put(`/order/${order_id}`, orderPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        (0, chai_1.expect)(response.data.quantity).to.equal(2);
        order_id = response.data.id;
    }));
});
describe('Decrease order count Route', () => {
    it('Devrait modifier une commande de produit', () => __awaiter(void 0, void 0, void 0, function* () {
        const orderPayload = {
            id: order_id,
            decrease: true
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.put(`/order/${order_id}`, orderPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        (0, chai_1.expect)(response.data.quantity).to.equal(1);
        order_id = response.data.id;
    }));
});
describe('Create review Route', () => {
    it('Devrait créer une review', () => __awaiter(void 0, void 0, void 0, function* () {
        const reviewPayload = {
            productId: product_id,
            rating: 5,
            comment: "Excellent produit"
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.post('/review', reviewPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        review_id = response.data.id;
    }));
});
describe('Get all reviews Route', () => {
    it('Devrait récupérer toutes les reviews', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.get(`/reviews`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.be.an('array');
        (0, chai_1.expect)(response.data).to.have.lengthOf.to.greaterThanOrEqual(1);
    }));
});
describe('Update review Route', () => {
    it('Devrait mettre à jour une review', () => __awaiter(void 0, void 0, void 0, function* () {
        const reviewPayload = {
            rating: 2,
            description: "En fait c'est pas si bien"
        };
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.put(`/review/${review_id}`, reviewPayload);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('id');
        (0, chai_1.expect)(response.data.rating).to.equal(2);
        (0, chai_1.expect)(response.data.description).to.equal("En fait c'est pas si bien");
    }));
});
describe('Delete review Route', () => {
    it('Devrait supprimer une review', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.delete(`/review/${review_id}`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('message');
        (0, chai_1.expect)(response.data.message).to.equal('Review deleted');
    }));
});
describe('Remove Order Route', () => {
    it('Devrait supprimer une commande', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.delete(`/order/${order_id}`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('message');
        (0, chai_1.expect)(response.data.message).to.equal('Order deleted');
    }));
});
describe('Remove Product Route', () => {
    it('Devrait supprimer un produit', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.delete(`/products/${product_id}`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('message');
        (0, chai_1.expect)(response.data.message).to.equal('Product deleted');
    }));
});
describe('Remove User Route', () => {
    it('Devrait supprimer un utilisateur', () => __awaiter(void 0, void 0, void 0, function* () {
        api.defaults.headers['Authorization'] = `Bearer ${token}`;
        const response = yield api.delete(`/user/${user_id}`);
        (0, chai_1.expect)(response.status).to.equal(200);
        (0, chai_1.expect)(response.data).to.have.property('message');
        (0, chai_1.expect)(response.data.message).to.equal('User deleted');
    }));
});
